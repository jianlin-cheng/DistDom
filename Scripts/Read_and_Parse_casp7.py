...
import pandas as pd
sequence_path = r'C:\Users\sajid\Downloads\CASP\CASP7\casp7.seq.txt'
domain_definition_csv_path = r'C:\Users\sajid\Downloads\CASP\CASP7\Domains Deffinition Summary - CASP7.csv'
save_path = r'C:\Users\sajid\Downloads\CASP\CASP7\casp7_dataset.csv'
file = open(sequence_path, mode = 'r', encoding = 'utf-8-sig')
lines = file.readlines()
text = file.read()
df = pd.read_csv(domain_definition_csv_path)
for i in range(0,len(df)):
    if('D' not in df['Domains'].iloc[i]):
        temp = df['Domains'].iloc[i]
        df['Domains'].iloc[i] = 'D1: ' + temp
...
for idx, contents in enumerate(parse):
    ...
    if len(domains!=0):
        ...
        print('Dom count: '+ str(dom_count))
        for i in range(0,len(domains)):
            if(domains.iloc[i].split(': ')[0]=='D0'):
                continue
            dom_name = domains.iloc[i].split(': ')[0]
            ...
            for dom in dom_range:
                ...
                # dom2.append(int(dom_start))
                # dom2.append(int(dom_end))

                print(dom_name, dom_start, dom_end)
            print(domain_definition_list)
            # indexes_to_pop = []
            # for idx,item in enumerate(dom2):
            #     if (idx == len(dom2)-1):
            #         break
            #     if(idx%2!=0 and dom2[idx+1]-dom2[idx] < 40):
            #         # domain_definition_list.pop(idx)
            #         # domain_definition_list.pop(idx+1)
            #         indexes_to_pop.append(idx)
            #         indexes_to_pop.append(idx+1)
            # for index in sorted(indexes_to_pop, reverse=True):
            #     del dom2[index]
            # for doms in dom2:
            #     domain_definition_list.append(doms)
